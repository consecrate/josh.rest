...
<MultipleChoice
  question="If $R = \\{(1,2), (2,3)\\}$ and $S = \\{(2,4), (3,5)\\}$, what is $S \\circ R$?"
  options={[
    "$\\{(1,4), (2,5)\\}$",
    "$\\{(1,2), (2,3), (2,4), (3,5)\\}$",
    "$\\{(1,4)\\}$",
  ...
  correct={0}
  explanation="Trace paths: $1 \\to 2$ (in R), then $2 \\to 4$ (in S) → add $(1,4)$. Also $2 \\to 3$ (in R), then $3 \\to 5$ (in S) → add $(2,5)$. The path $1 \\to 2 \\to 3$ doesn't continue in S from 2 (only from 3), so $(1,5)$ needs $1 \\to 3$, which isn't in R."
/>
...
<MultipleChoice
  question="If $R = \\{(1,2)\\}$ and $S = \\{(2,3)\\}$, what is $R \\circ S$?"
  options={[
    "$\\{(1,3)\\}$",
    "$\\{(2,2)\\}$",
    "$\\emptyset$",
  ...
  correct={2}
  explanation="$R \\circ S$ means: do $S$ first, then $R$. Starting from any element in $S$'s domain (2), we go $2 \\to 3$. Now we need $3 \\to ?$ in $R$... but $R$ has no pair starting from 3. Empty result!"
/>
...
<MultipleChoice
  ...
  correct={3}
  explanation="$1 \\to 2 \\to 3$ gives $(1,3)$. $2 \\to 3 \\to 1$ gives $(2,1)$. $3 \\to 1 \\to 2$ gives $(3,2)$. Also: $1 \\to 2 \\to 3 \\to 1$ in two steps? No, that's 3 steps. Wait, let's redo: $(1,3)$ from $1\\to 2\\to 3$. For $(1,1)$: need $1 \\to ? \\to 1$, we have $1\\to 2$ but no $2\\to 1$, and $1\\to 2$ but $2\\to 3$ not to 1. Hmm, actually: $3\\to 1$ and $1\\to 2$? That would be $(3,2)$. For $(1,1)$: we need path $1 \\to b \\to 1$. $1\\to 2$ and $2\\to ?\\to 1$? $2\\to 3$, $3\\to 1$. That's 3 steps. Actual $R^2$: $(1,3), (2,1), (3,2)$. So 'All of the above' is wrong!"
/>
...
<MultipleChoice
  question="To find $S \\circ R$ where $|R| = m$ and $|S| = n$, the naive algorithm takes:"
  options={[
    "$O(m + n)$",
    "$O(mn)$",
    "$O(m^2 + n^2)$",
