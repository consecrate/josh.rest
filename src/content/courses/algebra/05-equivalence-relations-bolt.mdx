...
$$
\underbrace{\{🔵, 🔵, 🔵\}}_{\text{Bucket 1}} \quad \underbrace{\{🔴, 🔴\}}_{\text{Bucket 2}} \quad \underbrace{\{🟢\}}_{\text{Bucket 3}}
$$
...
<MultipleChoice
  ...
  options={[
    "$\\begin{pmatrix} 1 & 1 & 0 \\\\ 1 & 1 & 0 \\\\ 0 & 0 & 1 \\end{pmatrix}$ — Two blocks",
    "$\\begin{pmatrix} 1 & 1 & 0 \\\\ 0 & 1 & 0 \\\\ 0 & 0 & 1 \\end{pmatrix}$ — Upper triangular",
    "$\\begin{pmatrix} 0 & 1 & 0 \\\\ 1 & 0 & 0 \\\\ 0 & 0 & 1 \\end{pmatrix}$ — Missing diagonal",
    "$\\begin{pmatrix} 1 & 1 & 1 \\\\ 1 & 1 & 0 \\\\ 1 & 0 & 1 \\end{pmatrix}$ — Asymmetric"
  ]}
  correct={0}
...
1. **Check Reflexive First** — Does $a R a$ for all $a$? 
   - $>$ fails immediately ($a > a$ is false)
   - $=$ passes ($a = a$ is true)

2. **Check Symmetric** — Is the relation "mutual"?
   - Divides ($|$) fails: $2 | 4$ but $4 \nmid 2$
   - Same remainder passes: if $a \equiv b$, then $b \equiv a$

3. **Transitivity usually follows** — If it's symmetric and "makes sense", transitivity often holds.

<MultipleChoice
  question="Is 'having a common factor greater than 1' an equivalence relation on $\\mathbb{Z}^+$?"
  options={[
    "Yes — it groups numbers by shared factors",
    "No — it fails reflexivity (1 has no common factor with itself > 1)",
    "No — it fails transitivity (4 and 6 share 2, 6 and 9 share 3, but 4 and 9 share nothing)",
...
**Partition → Relation:** Given buckets, two elements are related iff they're in the same bucket.

**Relation → Partition:** Given an equivalence relation, each equivalence class $[a]$ is a bucket.
