...
interface BinaryMultCarryProblem extends Problem {
  a: number;
  b: number;
  product: number;
}
...
/**
 * Check if a × b produces carries when adding partial products.
 * Returns true if any column has 2+ bits set among partial products.
 */
function hasCarries(a: number, b: number): boolean {
...
function generateWrongAnswers(correct: number, rng: () => number): number[] {
  ...
  const attempts = [
    ...
    () => correct ^ (1 << 3),                     // another carry mistake
  ];
...
export const binaryMultiplicationCarryGenerator: ProblemGenerator<BinaryMultCarryProblem> = {
  ...
  generate(seed: number): BinaryMultCarryProblem {
    const rng = mulberry32(seed);
    let a: number, b: number;

    // Rejection sampling to find a pair that HAS carries
    let attempts = 0;
    // Exclude the example 3×3 from problems
    const isExample = (x: number, y: number) => x === 3 && y === 3;
    do {
      a = Math.floor(rng() * 14) + 2; // 2-15 (avoid trivial 1×n)
      b = Math.floor(rng() * 14) + 2; // 2-15
      attempts++;
    } while ((!hasCarries(a, b) || isExample(a, b)) && attempts < 200);

    // Fallback to known-good pair with carries (5×3 = 15, has carries)
    if (!hasCarries(a, b)) {
      a = 5;
      b = 3;
    }
    ...
    const explanation =
      ...
      `Remember: $1 + 1 = 0$ carry $1$ in binary addition.`;

    return {
      question: `What is $${a} \\times ${b}$ in binary? (Watch for carries!)`,
...
export function runTests() {
  const gen = binaryMultiplicationCarryGenerator;
  ...
  // Test 3: hasCarries is correctly checked
  for (let i = 0; i < 50; i++) {
    const p = gen.generate(i);
    console.assert(hasCarries(p.a, p.b), `Seed ${i}: should have carries`);
  }
  console.log('✓ Has-carries constraint test passed');
