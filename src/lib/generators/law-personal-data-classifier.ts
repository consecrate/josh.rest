...
export const personalDataClassifier: ProblemGenerator = {
  ...
  generate(seed: number): Problem {
    ...
    const items = [
      ...
      {
        text: "Browser fingerprint (combination of screen resolution, fonts, plugins, timezone)",
        isPersonal: true,
        reason: "A browser fingerprint creates a unique identifier that can single out a specific user, even without cookies."
      },
      {
        text: "Aggregate statistic: '35% of visitors clicked the Buy button'",
        isPersonal: false,
        reason: "True aggregate data where no individual can be identified is not Personal Data."
      },
      {
        text: "An employee ID number (EMP-12345) used in an internal HR system",
        isPersonal: true,
        reason: "The employee ID links directly to an identifiable natural person within the company's systems."
      },
      {
        text: "A hashed email address (SHA-256) without the salt being stored",
        isPersonal: true,
        reason: "Unsalted hashes can be reversed using rainbow tables. Since re-identification is reasonably possible, it remains Personal Data."
      }
    ];
